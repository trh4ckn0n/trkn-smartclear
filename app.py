...
def scan_storage(size_limit=50, days_old=180):
    folders = ["/sdcard", "/storage/emulated/0"]
    files = []
    now = time.time()
...
# 🔹 Consultation de GPT-4 pour suggestions intelligentes
...
def main():
    ...
    files = scan_storage(size_limit=50, days_old=180)
    ...
    # 🔹 GPT-4 analyse les fichiers pour suggestions intelligentes
    gpt_advice = ask_gpt4(f"J'ai trouvé ces fichiers volumineux et anciens sur mon Android : {files[:5]} Que devrais-je faire ?")
    console.print(f"\n🤖 [bold cyan]Suggestion GPT-4 :[/bold cyan] {gpt_advice}\n")
    ...
    elif response == "💡 Demander conseil à GPT-4":
